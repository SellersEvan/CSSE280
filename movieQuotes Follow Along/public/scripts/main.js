...
class DetailPageController {
	...
	// init
	init() {
		SINGLE_MOVIE_QUOTES_MANAGER.openDatabase( this.update.bind( this ) );
	...
	updatePoster() {
		let title = SINGLE_MOVIE_QUOTES_MANAGER.movie;
		console.log( title )
		fetch( `https://www.omdbapi.com/?apikey=691ddc11&t=${ title }` ).then( res => {
			return res.json();
		}).then( data => {
			if ( data[ "Response" ] == "True" && data[ "Poster" ].length > 5 ) {
				let posterURL = data[ "Poster" ];
				let posterImg = document.querySelector( "#cardPoster" );
				posterImg.src = posterURL;
				posterImg.style.display = "flex";
			} else {
